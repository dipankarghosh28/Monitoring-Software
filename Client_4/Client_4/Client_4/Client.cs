...
namespace Client_4
{
    public partial class Client : Form
    {
        ...
        DataTable dtMain;
        private const int portNum = 4949; //Set the Port no. Here For eg. 4848 has been set.
        delegate void SetTextCallback(string text);
        string DBPath;
        TcpClient client;//Declaring it as a client side.
        NetworkStream ns;
        Thread t = null;
        private string hostName = Clipboard.GetText();

        public Client()
        {
            InitializeComponent();
            string iphostName = Dns.GetHostName(); // Retrive the Name of HOST
            string myIP = Dns.GetHostByName(iphostName).AddressList[0].ToString();// Get the IP
            textBox3.Text = myIP;
